...
int find_repetition(char *s, const std::vector<std::string> &strings)
{
    if (!s)
    ...
    // Check for substring matches within the first 6 characters
    std::string first_six = std::string(s).substr(0, 17);
    for (const std::string &substr : strings)
    {
        if (first_six.find(substr) != std::string::npos)
            return 0;
    }
    ...
    return 1; // No disqualifying conditions found
}
    ...
    do
    {
        ...
        if (continue_flag)
        {
            ...
            else
            {
                ...
                {
                    hextemp = key_mpz.GetBase16();
                    x = find_repetition(hextemp, strings);
