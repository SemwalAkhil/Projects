...
int main(int argc, char **argv)
{
	...
	{ // When none crypto is defined the default search is for Bitcoin
		FLAGCRYPTO = CRYPTO_BTC;
	...
	if (FLAGMODE != MODE_BSGS)
	{
		if (FLAG_N)
		{
			...
			{
				N_SEQUENTIAL_MAX = strtol(str_N, NULL, 16) * 1000000;
			}
			else
			{
				N_SEQUENTIAL_MAX = strtol(str_N, NULL, 10) * 16777216;
			}
	...
	if (FLAGMODE == MODE_BSGS)
	{
		...
		while (!feof(fd))
		{
			if (fgets(aux, 1022, fd) == aux)
			{
				trim(aux, " \t\n\r");
				if (strlen(aux) >= 128)
				{ // Length of a full address in hexadecimal without 04
					N++;
				}
				else
				{
		...
		if (!BSGS_AUX.IsZero())
		{ // If M is not divisible by  BSGS_GROUP_SIZE (1024)
		...
		if (!BSGS_R.IsZero())
		{ /* if BSGS_N modulo BSGS_M is not 0*/
			BSGS_N.Set(&BSGS_M);
		...
		if (FLAGSAVEREADFILE)
		{
			...
			{ /*Checking for old file    keyhunt_bsgs_3_   */
				snprintf(buffer_bloom_file, 1024, "keyhunt_bsgs_3_%" PRIu64 ".blm", bsgs_m);
				fd_aux1 = fopen(buffer_bloom_file, "rb");
				if (fd_aux1 != NULL)
				{
...
if (FLAGMODE != MODE_BSGS)
{
	...
	//======================================should be executed=================but not getting executed===========
	steps = (uint64_t *)calloc(NTHREADS, sizeof(uint64_t));
	checkpointer((void *)steps, __FILE__, "calloc", "steps", __LINE__ - 1);
	ends = (unsigned int *)calloc(NTHREADS, sizeof(int));
	checkpointer((void *)ends, __FILE__, "calloc", "ends", __LINE__ - 1);
...
int find_repetition(char *s)
{
	...
	{
		// printf("\nlocation 2\n");
		// std::cerr << "Error in implementing skip for " << s << std::endl;
		return 0; // Handle null input (skip null key)
	}
	// printf("\nlocation 3\n");
	// Check for 3 consecutive identical alphanumeric characters in `s`
	char previous = '\0';
	int count = 0;
	for (int i = 0; s[i] != '\0'; i++)
	{
		...
		if (isalnum(static_cast<unsigned char>(ch)))
		{
			if (ch == previous)
			{
				...
				{
					// printf("\nlocation 4\n");
					return 0; // Found 3 in a row (skip key with three in a row)
				}
			}
			else
			{
	...
	// printf("\nlocation 5\n");
	// Check for substring matches within the first 5 characters
	std::string st(s);
	std::string first_six = std::string(s).substr(0, 5);
	std::string last_six = std::string(s).substr(st.length() - 5);
	std::string line;
	while (std::getline(inFile, line))
	{
		if ((first_six.find(line) != std::string::npos) || (last_six.find(line) != std::string::npos))
		{
			// printf("\nlocation 6\n");
			// printf("\nnot skipping %s found %s\n", s, line.c_str());
			return 1; // (if key matches string from input.txt then execute)
		}
	}

	inFile.close();
	// printf("\nlocation 7\n");
	// Convert hexadecimal `s` to decimal
	// unsigned long long decimal_number;
	// try
	// {
	// 	decimal_number = std::stoull(s, nullptr, 16);
	// }
	// catch (...)
	// {
	...
	// Check for 5 consecutive identical digits in decimal string
	// std::string decimal_str = std::to_string(decimal_number);
	// previous = '\0';
	// count = 0;
	// for (char ch : decimal_str)
	// {
	// 	if (ch == previous)
	// 	{
	// 		count++;
	// 		if (count >= 2)
	// 			return 0; // Found 5 in a row
	// 	}
	// 	else
	// 	{
	// 		previous = ch;
	...
	return 0; // (if key not matched in input.txt then skip)
}
...
void *thread_process(void *vargp)
{
	...
	free(tt);
	grp->Set(dx);
	// std::vector<std::string> strings = {"1460", "2930", "68f3", "c936", "1764", "764f", "3080", "080d", "5749", "749f", "d2c5", "2c55", "1ba5", "ba53", "a534", "2de4", "de40", "e40f", "556e", "56e5", "6e52", "dc2a", "c2a0", "2a04", "1fa5", "fa5e", "a5ee", "5ee5", "3403", "4032", "0326", "326e", "6ac3", "ac38", "c387", "3875", "d916", "916c", "16ce", "6ce8", "17e2", "7e25", "e255", "2551", "551e", "3d94", "d94c", "94cd", "4cd6", "cd64", "7d4f", "d4fe", "4fe7", "fe74", "e747", "b862", "862a", "62a6", "2a62", "a62e", "1a96", "a96c", "96ca", "6ca8", "ca8d", "34a6", "4a65", "a659", "6591", "5911", "911d", "4aed", "aed2", "ed21", "d211", "2117", "1170", "9de8", "de82", "e820", "820a", "20a7", "0a7c", "1757", "7577", "5775", "7756", "756a", "56a9", "6a93", "2238", "2382", "382f", "82fa", "2fac", "facd", "acd0", "4b5f", "b5f8", "5f83", "f830", "8303", "303e", "03e9", "e9ae", "9ae4", "ae49", "e493", "4933", "933d", "33d6", "1538", "5386", "3869", "869a", "69ac", "9acc", "acc5", "cc5b", "2a22", "a221", "221c", "21c5", "1c58", "c58d", "58d8", "8d8f", "6bd3", "bd3b", "d3b2", "3b27", "b27c", "27c5", "7c59", "c591", "e02b", "02b3", "2b35", "b35a", "35a3", "5a35", "a358", "358f", "122f", "22fc", "2fca", "fca1", "ca14", "a143", "143c", "43c0", "3c05", "2ec1", "ec18", "c183", "1838", "8388", "388d", "88d5", "8d54", "d544", "6cd6", "cd61", "d610", "610b", "10b5", "0b53", "b53c", "53cb", "3cba", "ade6", "de6d", "e6d7", "6d7c", "d7ce", "7ce3", "ce3b", "e3b9", "3b9b", "1741", "7417", "4176", "176b", "76b0", "6b01", "b015", "015f", "15f4", "5f4d", "22bd", "2bd4", "bd43", "d43c", "43c2", "3c2e", "c2e9", "2e93", "e935", "9354", "7507", "5070", "070a", "70a1", "0a1a", "a1a0", "1a00", "a009", "009d", "09d4", "efae", "fae1
"c7f6", "7f64", "f64a", "64a0", "4a0e", "a0e8", "0e8b", "e8b1", "8b1f", "b1f4", "af55", "f55f", "55fc", "5fc5", "fc59", "c59c", "59c3", "9c33", "c335", "335c", "35c8", "5c8e", "c8ec", "8ec6", "ec67", "c67e", "67ed", "7ed2", "ed24", "d248", "2482", "4826", "16f1", "6f14", "f14f", "14fc", "4fc2", "fc20", "c205", "2054", "054c", "54cd", "4cd8", "cd87", "d87e", "87ee", "7ee6", "ee63", "e639", "6396", "396b", "96b3", "6b33", "b33d", "33df", "3df3", "35c0", "5c0d", "c0d7", "0d72", "d723", "7234", "234d", "34df", "4df7", "df7d", "f7de", "7deb", "deb0", "eb0f", "b0f2", "0f20", "f20c", "20cf", "0cf7", "cf70", "f706", "7062", "0624", "6244", "2444", "60f4", "0f4d", "f4d1", "4d11", "d115", "1157", "1574", "574f", "74f5", "4f5d", "f5de", "5dee", "deee", "eee4", "ee49", "e499", "4996", "9961", "961d", "61d9", "1d96", "d960", "9609", "609a", "09ac", "9ac6"};
	do
	{
		if (FLAGRANDOM)
		{
		...
		if (continue_flag) // checkpoint
		{
			hextemp = key_mpz.GetBase16();
			x = find_repetition(hextemp);
			// printf("\nloc1");
			count = 0;
			if (FLAGMATRIX)
			{
			...
			else
			{
				...
				if (FLAGQUIET == 0)
				{
					...
					// hextemp = key_mpz.GetBase16();
					// x = find_repetition(hextemp);
					printf("\rBase key: %s    %d\r", hextemp, x);
					fflush(stdout);
					// free(hextemp);
					THREADOUTPUT = 1;
				}
...
#if defined(_WIN64) && !defined(__CYGWIN__)
				WaitForSingleObject(write_random, INFINITE);
				key_mpz.Set(&n_range_start); // checkpoint
				n_range_start.Add(N_SEQUENTIAL_MAX);
				ReleaseMutex(write_random);
#else
				pthread_mutex_lock(&write_random);
...
void writekey(bool compressed, Int *key)
{
	...
	hextemp = key->GetBase16();
	if (strcmp(hextemp, "t") == 0)
	{
		printf("MANSI JAIN %s", hextemp);
	}
	else
	{
		printf("SKIP");
...
void writekeyeth(Int *key)
{
	...
	// MANSI JAIN COMMENTED THIS==================================================================================s
	// printf("\ Hit!!!! Private Key: %s\naddress: %s\n",hextemp,address);
#if defined(_WIN64) && !defined(__CYGWIN__)
	ReleaseMutex(write_keys);
#else
	pthread_mutex_unlock(&write_keys);
...
bool forceReadFileXPoint(char *fileName)
{
	...
	printf("[+]oops Allocating memory for %" PRIu64 " elements: %.2f MB\n", numberItems, (double)(((double)sizeof(struct address_value) * numberItems) / (double)1048576));
	addressTable = (struct address_value *)malloc(sizeof(struct address_value) * numberItems);
	checkpointer((void *)addressTable, __FILE__, "malloc", "addressTable", __LINE__ - 1);
