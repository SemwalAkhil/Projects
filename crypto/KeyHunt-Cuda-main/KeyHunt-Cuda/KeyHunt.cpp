...
void KeyHunt::FindKeyGPU(TH_PARAM *ph)
{
	...
	while (ok && !endOfSearch)
	{
		...
		if (ok)
		{
			// // Define the random step size range
			// const uint64_t MIN_STEP_SIZE = 1;		  // Minimum step size
			// const uint64_t MAX_STEP_SIZE = 268435456; // Maximum step size
			// 										  // 4FC8FC228
			// // Random number generator
			// std::random_device rd;	// Seed generator
			// std::mt19937 gen(rd()); // Mersenne Twister RNG
			// std::uniform_int_distribution<uint64_t> dis(MIN_STEP_SIZE, MAX_STEP_SIZE);
			// Int rangeLength;
			// rangeLength.Sub(&tRangeEnd, &tRangeStart);
			for (int i = 0; i < nbThread; i++)
			{

				keys[i].Add((uint64_t)STEP_SIZE); // ORIGINAL FFFF FFFF FFFC 0001, FFFFFFFFFFFC0001
				// Int randomStep(dis(gen)); // Create an Int object

				// keys[i].Add(&randomStep); // Increment the key by the random step

				// // Ensure the key stays within range
				// if (keys[i].IsGreater(&tRangeEnd))
				// {							   // If keys[i] > tRangeEnd
				// 	keys[i].Sub(&tRangeEnd);   // keys[i] = keys[i] - tRangeEnd
				// 	keys[i].Mod(&rangeLength); // keys[i] %= (tRangeEnd - tRangeStart)
				// 	keys[i].Add(&tRangeStart); // keys[i] += tRangeStart
				// }
				std::cout << "Current Key : " << keys[i].GetBase16() << std::endl;
			}
