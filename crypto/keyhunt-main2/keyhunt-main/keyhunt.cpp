...
std::pair<Int, Int> rangeOps(char *range_start, char *range_end)
{
	...
	if (n_range_start.IsEqual(&n_range_end) == false)
	{
		if (n_range_start.IsLower(&secp->order) && n_range_end.IsLowerOrEqual(&secp->order))
		{
			...
			{
				fprintf(stderr, "[A] (line454) Opps, start range can't be great than end range. Swapping them\n");
...
void *thread_process(void *vargp)
{
	...
	std::string rangeStarts;
	std::string rangeEnds;
	char *rangeS = new char[1000];
	char *rangeE = new char[1000];
	std::pair<Int, Int> range;
	Tokenizer t;
	char *rangeStr = new char[1000];
	do
	{
		rangeStarts = readLineFromFile("start.txt", fileCount);
		rangeEnds = readLineFromFile("end.txt", fileCount);
		// printf("\n\n[A] from :%s \n\n[A] to   :%s\n\n[A] count :%d\n", rangeStarts.c_str(), rangeEnds.c_str(), fileCount);
		strcpy(rangeS, rangeStarts.c_str());
		strcpy(rangeE, rangeEnds.c_str());
		strcpy(rangeStr, (std::string(trim(rangeS, " \t\n\r")) + " : " + std::string(trim(rangeE, " \t\n\r"))).c_str());
		stringtokenizer(rangeStr, &t);
		range_start = nextToken(&t);
		range_end = nextToken(&t);
		fileCount++;
		if ((rangeStarts == "") || (rangeEnds == ""))
		{
			// printf("files completed ending loop \n");
			break;
		}
		// printf("%s : %s\n", rangeStarts.c_str(), rangeEnds.c_str());
		// strcpy(range_start, rangeStarts.c_str());
		// strcpy(range_end, rangeEnds.c_str());
...
bool forceReadFileAddress(char *fileName)
{
	...
	if (fileDescriptor == NULL)
	{
		fprintf(stderr, "[E] Error opening the (BTC) file %s, line %i\n", fileName, __LINE__ - 2);
		return false;
	}
