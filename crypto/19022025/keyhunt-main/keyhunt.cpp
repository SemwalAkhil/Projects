...
Secp256K1 *secp;
// check point
std::string readLineFromFile(const std::string &filePath, int index)
{
	std::ifstream file(filePath);
	if (!file.is_open())
	{
		throw std::runtime_error("[A] (line420) Unable to open file: " + filePath); // Improved error handling
	}
	...
	for (int i = 0; i <= index; ++i)
	{
		if (!std::getline(file, line))
		{
			if (i < index)
			{
				throw std::out_of_range("[A] (line430) Index out of range"); // Handle index exceeding line count
			}
			return ""; // Return empty string if index is the last line and file ends
		}
	}
	file.close();
	return line;
}
std::pair<Int, Int> rangeOps(char *range_start, char *range_end)
{
	// std::cout << "Range start and end : " << range_start << " : " << range_end << std::endl;
	Int n_range_start;
	Int n_range_end;
	Int n_range_aux; // Declare n_range_aux
	Int n_range_diff;
	...
	if (n_range_start.IsEqual(&n_range_end) == false)
	{
		if (n_range_start.IsLower(&secp->order) && n_range_end.IsLowerOrEqual(&secp->order))
		{
			...
			{
				fprintf(stderr, "[A] (line454) Opps, start range (%s) can't be great than end range (%s). Swapping them\n", range_start, range_end);
			...
			return std::make_pair(n_range_start, n_range_end);
		}
		else
		{
			std::cerr << "[A] (line465) Range outside valid bounds\n";
			return std::make_pair(Int(), Int());
		}
	}
	else
	{
		fprintf(stderr, "[A] (line471) Start and End range can't be the same\nFallback to random mode!\n");
		// FLAGRANGE = 0;
		return std::make_pair(Int(), Int());
	}
}
...
int main(int argc, char **argv)
{
	...
	size_t rsize;
	// check point
	std::string rangeStarts;
	std::string rangeEnds;
		...
		case 'r':
			if (optarg != NULL)
			{
				...
				case 1:
					...
					// rangeStarts = readLineFromFile("start.txt", count);
					// count++;
					// strcpy(range_start, rangeStarts.c_str());
				...
				case 2:
					...
					// rangeStarts = readLineFromFile("start.txt", count);
					// rangeEnds = readLineFromFile("end.txt", count);
					// count++;
					// strcpy(range_start, rangeStarts.c_str());
					// strcpy(range_end, rangeEnds.c_str());
...
void *thread_process(void *vargp)
{
	...
	long unsigned int numLines1 = countLines("start.txt");
	long unsigned int numLines2 = countLines("end.txt");

	for (long unsigned int count1 = 0; count1 < numLines1; count1++)
	{
		// read starting characters
		rangeStarts = trim(readLineFromFile("start.txt", count1));
		// if empty string then continue to next iteration
		if (rangeStarts.length() == 0)
		{
			continue;
		}
		for (long unsigned int count2 = 0; count2 < numLines2; count2++)
		{
			// read ending characters
			rangeEnds = trim(readLineFromFile("end.txt", count2));
			if (rangeEnds.length() == 0)
			{
				continue;
			}
			// assign merged string to rangeSt
			rangeSt.assign(rangeStarts)
				.append(rangeEnds);
			// assign values to rangeS and rangeE of Int Class
			rangeS.SetBase16(rangeSt.c_str());
			rangeE.SetBase16(rangeSt.c_str());
			// rangeE = rangeS + 100
			rangeE.Add(uint64_t(100));
			// convert ranges to string to perform further operations
			range_start = rangeS.GetBase16();
			range_end = rangeE.GetBase16();
			range = rangeOps(range_start, range_end);
			// printf("\nrange_start:|%s|\nrange_end:|%s|\nrangeStarts:|%s|\nrangeEnds:|%s|\n", rangeS.GetBase16(), rangeE.GetBase16(), rangeStarts.c_str(), rangeEnds.c_str());
			// set range to perform operations and the continue flag after for each pair of ranges
			n_range_start = range.first;
			n_range_end = range.second;
			continue_flag = 1;
