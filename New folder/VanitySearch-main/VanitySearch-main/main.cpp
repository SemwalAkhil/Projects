...
#include <map> // For counting digit frequencies
...
{
  // Unordered set to store numbers to skip (read from file)
  std::unordered_set<string> skipNumbers;
  ...
  while (getline(inFile, line))
  {
    ...
    if (line.length() > 0)
    {
      ...
      // Check if the number is in the skipNumbers set
      if (skipNumbers.find(line) != skipNumbers.end())
      {
        continue; // Skip this number if it's already in the set
      }

      // Check if the number has a digit repeated more than twice
      if (hasRepeatedDigits(line))
      {
        continue; // Skip this number if it has repeated digits
      }

      // If both conditions pass, add the line to the lines vector
      lines.push_back(line);
      nbLine++;

      // Progress display (for large files)
      if (loaddingProgress && (nbLine % 50000) == 0)
      {
